...
Proxy verification: the pinned `authlib-injector-1.2.8.jar` is mounted as a Java agent on BungeeCord and all four vanilla worlds, pointing to the exact configured Yggdrasil root `https://skin.mualliance.ltd/api/yggdrasil`. **Only the proxy** uses `online_mode: true`; the four vanilla backend `server.properties` deliberately keep `online-mode=false` and are reachable through the private proxy network only. Do not flip backend online mode to `true` or enable legacy Bungee `ip_forward` on vanilla. Log in to the same Yggdrasil service through a compatible Minecraft 26.3 launcher before joining; a locally invented offline name is rejected. Changing the auth service requires a reviewed identity migration, not just a JVM flag.

## QQ identity and administrator roles

The proxy plugin is built reproducibly from `plugins/identity/src/XduIdentity.java` against the pinned BungeeCord jar. QQ registrations and current connections are kept in `volumes/event/proxy/plugins/XduIdentity/`. An unregistered non-admin sees a persistent red `/qq XXX` title; native idle subtitles are suppressed until QQ is confirmed. `/qq <signup QQ>` displays private clickable green `[是]` and red `[不是]` choices. No draft is persisted until `[是]`; a confirmed QQ cannot be overwritten with `/qq`. Duplicate QQ, malformed identifiers and unauthorized backend entry are rejected. Login is Yggdrasil-authenticated **at the proxy**; QQ is only an event signup key, not a substitute for login or proof of QQ ownership. The verified proxy UUID keys QQ and session-scoped admin authorization. Vanilla backend player records still use the name-derived native offline UUID; the race archive records both identifiers rather than conflating them. Existing offline-mode test registrations and admin/OP grants were backed up before the cutover and cleared; players must re-register QQ and host admins must grant roles again after authenticated login.

Grant commentator/OP for the **current authenticated connected session** from the trusted host using `./scripts/racectl admin Name A` (or B/C). A trusted native `xdu_commentary_A/B/C` tag may select the default group but cannot grant permission by itself. Roles and OP must be reapproved by the host after a reconnect; OP is revoked when the session leaves a server or disconnects. Commentators are spectators in race worlds and excluded from rosters and scoring. Player-supplied QQ never grants permissions. A vanilla backend's native UUID is not a proxy login credential; backends must stay inaccessible to players except through the authenticated proxy. Registration repair is console-only, with the player disconnected and no active GP.
